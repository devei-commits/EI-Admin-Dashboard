...
const GoodReceiving: React.FC = () => {
 ...
 const getConditionColor = (condition: string) => {
  switch (condition) {
   ...
   default:
    return 'bg-slate-50 text-slate-300 border-gray-200';
  }
 };
  ...
  <div className="space-y-6">
   ...
   <div className="flex items-center justify-between">
    <div>
     ...
     <p className="text-sm text-slate-400 mt-1">Track and manage received goods from suppliers</p>
    </div>
    <button
   ...
   <div className="grid gap-4">
    {filteredRecords.length === 0 ? (
     <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
      ...
      <p className="text-slate-500 font-medium">No receiving records found</p>
      ...
      <div key={record.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
       ...
       <div className="p-4 border-b border-gray-200 flex items-center justify-between">
        <div className="flex-1">
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
           <p className="text-slate-400 text-xs uppercase tracking-wide">Order ID</p>
           <p className="text-gray-800 font-semibold">{record.orderId}</p>
          </div>
          <div>
           <p className="text-slate-400 text-xs uppercase tracking-wide">Supplier</p>
           <p className="text-gray-800 font-semibold">{record.supplierName}</p>
          </div>
          <div>
           <p className="text-slate-400 text-xs uppercase tracking-wide">Received Date</p>
           <p className="text-gray-800 font-semibold">{record.receivedDate}</p>
          </div>
          <div>
           <p className="text-slate-400 text-xs uppercase tracking-wide">Received By</p>
        ...
        >
         <svg className={`w-5 h-5 text-slate-400 transition-transform ${selectedRecord?.id === record.id ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
       ...
       {selectedRecord?.id === record.id && (
        <div className="p-4 space-y-4">
         {/* Record Info */}
         <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-3 bg-slate-50 rounded-lg">
          <div>
          ...
          <div>
           <p className="text-xs text-slate-400 uppercase tracking-wide">Total Items</p>
          ...
          <table className="w-full text-sm">
           <thead>
            <tr className="border-b border-gray-200 bg-slate-50">
             <th className="px-3 py-2 text-left text-xs font-semibold text-slate-300 uppercase">Item Name</th>
             <th className="px-3 py-2 text-center text-xs font-semibold text-slate-300 uppercase">Ordered Qty</th>
             <th className="px-3 py-2 text-center text-xs font-semibold text-slate-300 uppercase">Received Qty</th>
             <th className="px-3 py-2 text-center text-xs font-semibold text-slate-300 uppercase">Unit</th>
             <th className="px-3 py-2 text-left text-xs font-semibold text-slate-300 uppercase">Condition</th>
             <th className="px-3 py-2 text-left text-xs font-semibold text-slate-300 uppercase">Notes</th>
            </tr>
           </thead>
           <tbody>
            {record.items.map((item) => (
             <tr key={item.id} className="border-b border-gray-200 hover:bg-slate-50">
              ...
              <td className="px-3 py-2 text-center text-slate-300">{item.orderedQty}</td>
              <td className="px-3 py-2 text-center">
               <span className={item.receivedQty === item.orderedQty ? 'text-green-600 font-semibold' : 'text-slate-800 font-semibold'}>
                {item.receivedQty}
               </span>
              </td>
              ...
              <td className="px-3 py-2 text-slate-400 text-sm">{item.notes}</td>
             </tr>
            ))}
           </tbody>
          </table>
         ...
         <div className="flex gap-2 pt-2">
          ...
          <button className="flex-1 px-4 py-2 bg-slate-500 hover:bg-gray-600 text-white text-sm font-medium rounded-lg transition-colors">
           Print
          </button>
         </div>
        </div>
   ...
   <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
    <div className="bg-white rounded-lg p-4 border border-gray-200">
     <p className="text-xs text-slate-400 uppercase tracking-wide mb-1">Total Records</p>
