...
export function PISDetailsDialog({ pis, currentRole, isOpen, onClose, isEmbedded = false }: PISDetailsDialogProps) {
  ...
  const handleTakeAction = async (decision?: string, comments?: string) => {
    ...
    if (taskType === 'BD') {
      ...
      } else if (pis.stage === 'BD_WFP_REVIEW') {
      ...
      if (currentRole === 'RND_LEAD' && pis.stage === 'RND_LEAD_REVIEW') {
        toStage = 'BD_WFP_REVIEW';
      } else if (pis.stage === 'RND_DEVELOPMENT') {
        toStage = 'RND_APPROVAL';
      } else if (pis.stage === 'RND_APPROVAL') {
        toStage = 'RND_REVIEW_CORRECTIONS';
      } else if (currentRole === 'RND_LEAD' && pis.stage === 'RND_REVIEW_CORRECTIONS') {
        toStage = 'QUALITY_REVIEW';
      } else if (currentRole === 'RND_STAFF' && pis.stage === 'RND_REVIEW_CORRECTIONS') {
        toStage = 'RND_REVIEW_CORRECTIONS';
      }
    } else if (taskType === 'QA') {
  ...
  const openDecisionDialog = () => {
    ...
    } else if (taskType === 'BD' && pis.stage === 'BD_WFP_REVIEW') {
    ...
    } else if (taskType === 'RND' && pis.stage === 'RND_DEVELOPMENT') {
      stageDecision = 'Development complete; moved to R&D approval & timeline';
    } else if (taskType === 'RND' && pis.stage === 'RND_APPROVAL') {
      stageDecision = 'Sample submitted for R&D review';
    } else if (taskType === 'RND' && pis.stage === 'RND_REVIEW_CORRECTIONS') {
                            ...
                            <div className="space-y-1">
                              <div className="flex items-center gap-2">
                                <Badge variant="outline">
                                  {stageChange
                                    ? `${entry.fromStage ? getStageLabel(entry.fromStage) : '—'} → ${
                                        entry.toStage ? getStageLabel(entry.toStage) : '—'
                                      }`
                                    : 'Activity'}
                                </Badge>
                      ...
                      <div key={a.id} className="flex items-center justify-between gap-3 p-3 border rounded-lg bg-white">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-gray-500">
                            {a.category ? String(a.category).replace(/_/g, ' ') : '—'}
