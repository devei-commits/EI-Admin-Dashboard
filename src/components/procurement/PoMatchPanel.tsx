    ...
    <div className={`rounded-xl border border-border bg-surface ${locked ? 'opacity-60' : ''}`}>
      <div className="px-4 py-3 border-b border-hairline flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] tracking-wide text-ink-3 uppercase">3-way match</span>
          <span className={`text-[11px] px-2 py-0.5 rounded border font-semibold ${vcfg.bg} ${vcfg.text} ${vcfg.border}`}>{vcfg.label}</span>
          {isOverridden && (
            <span className="text-[11px] px-2 py-0.5 rounded border font-semibold bg-warn-soft text-warn border-[color:var(--st-amber-fg)]/30">Overridden</span>
          )}
          {state.closure.isClosed && (
            <span className="text-[11px] px-2 py-0.5 rounded border font-semibold bg-ok-soft text-ok border-[color:var(--st-green-fg)]/30">Closed</span>
          )}
        </div>
      </div>
      ...
      <div className="px-4 py-3 space-y-3">
                ...
                return (
                  <tr key={`${ln.code}-${i}`}>
                    <td className="px-2.5 py-1.5">
                      <div className="font-medium text-ink">{ln.name}</div>
                      <div className="text-[9.5px] text-ink-4 font-mono">{ln.code}{ln.unit ? ` · ${ln.unit}` : ''}</div>
                    </td>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
          <div className="rounded-md border border-border px-2.5 py-1.5"><div className="text-ink-4">PO total</div><div className="font-semibold text-ink tabular-nums">{inr(t.poTotal)}</div></div>
          <div className="rounded-md border border-border px-2.5 py-1.5"><div className="text-ink-4">Received payable</div><div className="font-semibold text-ink tabular-nums">{inr(t.receivedPayable)}</div></div>
          <div className="rounded-md border border-border px-2.5 py-1.5"><div className="text-ink-4">Invoice</div><div className="font-semibold text-ink tabular-nums">{inr(t.invoiceAmount)}</div></div>
          <div className={`rounded-md border px-2.5 py-1.5 ${t.amountOk ? 'border-border' : 'border-[color:var(--st-red-fg)]/30 bg-err-soft'}`}>
            <div className="text-ink-4">Amount variance</div>
            <div className={`font-semibold tabular-nums ${t.amountOk ? 'text-ink' : 'text-err'}`}>{t.invoiceAmount == null ? '—' : `${t.amountVariancePct > 0 ? '+' : ''}${t.amountVariancePct}%`}</div>
          </div>
        </div>
        ...
        {state.closure.isClosed ? (
          ...
          <>
            ...
            <div className="rounded-md border border-border px-3 py-2.5 space-y-2">
              <div className="flex items-center gap-1.5 text-[11px] font-bold text-ink-3"><FileText className="h-3.5 w-3.5" /> Vendor invoice</div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                <input value={invNo} onChange={(e) => setInvNo(e.target.value)} placeholder="Invoice no." aria-label="Invoice no." className="rounded-md border border-border px-2 py-1.5 text-[12px]" />
                <input type="date" value={invDate} onChange={(e) => setInvDate(e.target.value)} aria-label="Invoice date" className="rounded-md border border-border px-2 py-1.5 text-[12px]" />
                <input value={invAmount} onChange={(e) => setInvAmount(e.target.value)} inputMode="decimal" placeholder="Amount (incl. GST)" aria-label="Amount (incl. GST)" className="rounded-md border border-border px-2 py-1.5 text-[12px]" />
              </div>
              <button
                type="button"
            ...
            {state.hasInvoice && verdict === 'variance' && !isOverridden && (
              <div className="rounded-md border border-[color:var(--st-red-fg)]/30 bg-err-soft px-3 py-2.5 space-y-2">
                <div className="flex items-center gap-1.5 text-[11px] font-bold text-err"><AlertTriangle className="h-3.5 w-3.5" /> Match variance — payment &amp; close are blocked</div>
                <p className="text-[11px] text-err">Fix the invoice/GRN, or override with a justification to proceed.</p>
                <input value={overrideNote} onChange={(e) => setOverrideNote(e.target.value)} placeholder="Justification (required to override)…" aria-label="Justification (required to override)" className="w-full rounded-md border border-[color:var(--st-red-fg)]/30 px-2 py-1.5 text-[12px]" />
                <button
                  type="button"
            ...
            {state.matchCleared && (
              <div className="rounded-md border border-[color:var(--st-green-fg)]/30 bg-ok-soft px-3 py-2.5 space-y-2.5">
                <div className="flex items-center gap-1.5 text-[11px] font-bold text-ok"><CheckCircle2 className="h-3.5 w-3.5" /> Match cleared — pay via Treasury &amp; close</div>

                {/* Treasury linkage (Flowchart Sub-flow I → Treasury outward) */}
                {state.sentToTreasury && state.treasury ? (
                  <div className="rounded-md border border-brand-soft bg-brand-soft px-3 py-2 text-[12px] text-brand flex items-center gap-2 flex-wrap">
                    <Landmark className="h-4 w-4 shrink-0" />
                    <span>Sent to Treasury · <span className="font-mono font-bold">{state.treasury.outwardCode}</span></span>
                    <span className="px-1.5 py-0.5 rounded border border-brand-soft bg-surface text-[10px] font-semibold uppercase tracking-wide">{state.treasury.status}</span>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-[10px] text-ink-3">Treasury runs the cash-flow gate + approval; the PO is stamped paid on execution.</span>
                  </div>
                ) : null}

                {state.payment.isPaid ? (
                  <div className="text-[12px] text-ok flex items-center gap-1.5">
                  ...
                  <div className="space-y-1">
                    <div className="text-[10px] text-ink-3">Or record a payment made outside Treasury:</div>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      <select value={payMode} onChange={(e) => setPayMode(e.target.value as PoPaymentMode)} aria-label="Payment mode" className="rounded-md border border-border px-2 py-1.5 text-[12px]">
                        {PO_PAYMENT_MODE_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                      </select>
                      <input value={payTxn} onChange={(e) => setPayTxn(e.target.value)} placeholder="UTR / ref" aria-label="UTR / ref" className="rounded-md border border-border px-2 py-1.5 text-[12px]" />
                      <input value={payAmount} onChange={(e) => setPayAmount(e.target.value)} inputMode="decimal" placeholder={`Amount (${t.invoiceAmount ?? ''})`} aria-label="Payment amount" className="rounded-md border border-border px-2 py-1.5 text-[12px]" />
                      <button
                        type="button"
                        ...
                        className={`${btnBase} bg-ok text-white hover:brightness-95 justify-center`}
                      >
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-[color:var(--st-green-fg)]/30">
                  <input value={closeNote} onChange={(e) => setCloseNote(e.target.value)} placeholder="Closure note (optional)…" aria-label="Closure note (optional)" className="flex-1 min-w-[160px] rounded-md border border-border px-2 py-1.5 text-[12px]" />
                  <button
                    type="button"
                ...
                {!state.payment.isPaid && (
                  <p className="text-[10px] text-ink-3">Net terms? You can close now and pay later — payment is recorded independently.</p>
                )}
              </div>
            )}
          </>
