      ...
      >
        <div className="px-5 py-4 border-b border-border bg-surface-3 flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-[11px] font-semibold text-ink-3 uppercase tracking-wide">GRN stage timeline</p>
            <h2 id="grn-stage-timeline-title" className="text-lg font-bold text-ink mt-0.5 font-mono">
              {row.grnNo}
            </h2>
          ...
          <button type="button" onClick={onClose} className="shrink-0 px-2 py-1 rounded-lg border border-border text-ink-3 text-sm hover:bg-surface-3" aria-label="Close">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-auto px-5 py-4">
                  ...
                  <li key={stage} className="flex gap-3 pb-4 last:pb-0">
                    <div className="flex flex-col items-center">
                      ...
                      {idx < GRN_STAGE_ORDER.length - 1 && (
                        <div className={`w-0.5 flex-1 min-h-[24px] ${done ? 'bg-brand' : 'bg-surface-3'}`} />
                      )}
                    </div>
                      ...
                      <p className="text-[11px] text-ink-3 mt-1">
                        {recorded?.at ? (
                          <>
                            ...
                            {recorded.actor ? <span className="text-ink-4"> · {recorded.actor}</span> : null}
                          </>
                        ) : done ? (
                          <span className="text-ink-4">Recorded (time unavailable)</span>
                        ) : (
                          <span className="text-ink-4">Pending</span>
                        )}
                      </p>
                    </div>
                  </li>
          ...
          {succeeded && !error ? (
            <p className="mt-3 text-xs text-ok font-semibold">Stage advanced successfully.</p>
          ) : null}
        </div>

        <div className="px-5 py-3 border-t border-border bg-surface flex flex-wrap items-center justify-end gap-2">
          <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg border border-border text-ink-2 text-sm font-semibold hover:bg-surface-3">
            Close
          </button>
          {nextStage && !loading ? (
            <button
