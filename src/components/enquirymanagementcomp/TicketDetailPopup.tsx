...
const ActivityTimeline: React.FC<ActivityTimelineProps> = ({ activities, isLoading }) => {
  const getActivityIcon = (type: TicketActivity['type']) => {
    const iconMap: Record<TicketActivity['type'], { icon: string; color: string }> = {
      ...
      'status-change': { icon: '📊', color: 'bg-amber-100 text-amber-600' },
      'priority-change': { icon: '⚡', color: 'bg-orange-100 text-orange-600' },
            ...
            >
              ...
              >
                ...
                <div className={`flex items-center justify-between mt-2 text-xs ${
                  msg.senderType === 'staff' && !msg.isInternal ? 'text-amber-100' : 'text-gray-500'
                }`}>
      ...
      <div className="border-t border-gray-200 pt-4">
        ...
        <div className="flex gap-2">
          <textarea
            ...
            className={`flex-1 px-4 py-3 border rounded-xl resize-none focus:ring-2 focus:ring-amber-500 focus:border-transparent ${
          ...
          <button
            ...
            className="px-4 py-2 bg-amber-500 text-white rounded-xl hover:bg-amber-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors self-end"
          >
