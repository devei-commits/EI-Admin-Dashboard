...
const StatCard: React.FC<StatCardProps> = ({ 
 ...
 const colorClasses = {
  blue: 'bg-brand-soft border-brand',
  amber: 'bg-warn-soft border-warn',
  emerald: 'bg-ok-soft border-ok',
  red: 'bg-err-soft border-err',
  purple: 'bg-purple-50 border-purple-200',
  gray: 'bg-surface-2 border-border',
 };

 const iconBgClasses = {
  blue: 'bg-blue-200 text-brand',
  amber: 'bg-surface-3 text-ink',
  emerald: 'bg-emerald-200 text-ok',
  red: 'bg-red-200 text-err',
  ...
  >
   <div className="flex items-start justify-between">
    <div className="flex-1">
     ...
     {trend && (
      <div className={`flex items-center gap-1 mt-2 text-sm ${trend.isPositive ? 'text-ok' : 'text-err'}`}>
...
const PriorityBadge: React.FC<{ priority: TicketPriority; count: number }> = ({ priority, count }) => {
 const colors = {
  ...
  medium: 'bg-brand-soft text-brand border-brand',
  high: 'bg-warn-soft text-warn border-warn',
  urgent: 'bg-err-soft text-err border-err',
 };
...
const StatusProgress: React.FC<StatusProgressProps> = ({ stats, total }) => {
 const statusConfig: { key: keyof typeof stats; label: string; color: string }[] = [
  { key: 'new', label: 'New', color: 'bg-brand' },
  { key: 'open', label: 'Open', color: 'bg-ink' },
  { key: 'inProgress', label: 'In Progress', color: 'bg-purple-500' },
  { key: 'pendingCustomer', label: 'Pending Customer', color: 'bg-orange-400' },
  { key: 'pendingInternal', label: 'Pending Internal', color: 'bg-warn' },
  { key: 'resolved', label: 'Resolved', color: 'bg-ok' },
  { key: 'closed', label: 'Closed', color: 'bg-gray-400' },
 ];

 return (
  <div className="space-y-4">
   <div className="h-4 flex rounded-full overflow-hidden bg-surface-3">
   ...
   <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
    {statusConfig.map(({ key, label, color }) => (
     <div key={key} className="flex items-center gap-2 text-sm">
      ...
      <span className="font-semibold text-ink ml-auto">{stats[key]}</span>
     </div>
    ))}
   </div>
  </div>
   ...
   <table className="w-full">
    <thead className="sticky top-0 z-20">
     <tr className="[&_th]:bg-surface-2 border-b border-border">
      <th scope="col" className="text-left py-3 px-4 text-xs font-semibold text-ink-3 uppercase tracking-wide">Staff</th>
      <th scope="col" className="text-center py-3 px-4 text-xs font-semibold text-ink-3 uppercase tracking-wide">Active</th>
      <th scope="col" className="text-center py-3 px-4 text-xs font-semibold text-ink-3 uppercase tracking-wide">Resolved Today</th>
      <th scope="col" className="text-center py-3 px-4 text-xs font-semibold text-ink-3 uppercase tracking-wide">Avg Time</th>
      <th scope="col" className="text-center py-3 px-4 text-xs font-semibold text-ink-3 uppercase tracking-wide">Rating</th>
      <th scope="col" className="text-center py-3 px-4 text-xs font-semibold text-ink-3 uppercase tracking-wide">Overdue</th>
     </tr>
    </thead>
      ...
      >
       <td className="py-3 px-4">
        <div className="flex items-center gap-3">
         <div className="w-9 h-9 rounded-full bg-ink flex items-center justify-center text-white font-semibold text-sm">
          {staff.staffName.split(' ').map(n => n[0]).join('')}
         </div>
         <div>
          <p className="font-medium text-ink">{staff.staffName}</p>
          <p className="text-xs text-ink-3">{staff.department}</p>
         </div>
        </div>
       </td>
       <td className="py-3 px-4 text-center">
        <span className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-brand-soft text-brand font-semibold">
         {staff.activeTickets}
        </span>
       </td>
       <td className="py-3 px-4 text-center">
        <span className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-ok-soft text-ok font-semibold">
         {staff.resolvedToday}
        </span>
       </td>
       <td className="py-3 px-4 text-center">
        <span className="text-ink-2 font-medium">{staff.avgResolutionTime}h</span>
       </td>
       <td className="py-3 px-4 text-center">
        <div className="flex items-center justify-center gap-1">
         <svg className="w-4 h-4 text-warn" fill="currentColor" viewBox="0 0 20 20">
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
         </svg>
         <span className="font-semibold text-ink">{staff.satisfactionScore}</span>
        </div>
       </td>
       <td className="py-3 px-4 text-center">
        {staff.overdueTickets > 0 ? (
         <span className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-err-soft text-err font-semibold">
  ...
  <div className="space-y-4">
   <div className="flex items-center gap-4 justify-end">
    ...
    <div className="flex items-center gap-2 text-sm">
     ...
     <span className="text-ink-2">Resolved</span>
    </div>
   </div>
    ...
    {data.map((item, index) => (
     <div key={index} className="flex-1 flex flex-col gap-1">
      <div className="flex gap-1 items-end flex-1">
       <div 
        className="flex-1 bg-blue-400 rounded-t transition-all hover:bg-brand"
        style={{ height: `${(item.created / maxValue) * 100}%` }}
        title={`Created: ${item.created}`}
       />
       <div 
        className="flex-1 bg-emerald-400 rounded-t transition-all hover:bg-ok"
        style={{ height: `${(item.resolved / maxValue) * 100}%` }}
        title={`Resolved: ${item.resolved}`}
       />
      </div>
      <span className="text-xs text-ink-3 text-center truncate">
 ...
 return (
  <div className="space-y-4">
   <div className="grid grid-cols-3 gap-4">
    <div className="text-center p-4 bg-ok-soft rounded-xl border border-ok">
     <p className="text-2xl font-bold text-ok">{metrics.onTime}</p>
     <p className="text-sm text-ok font-medium">On Time</p>
    ...
    <div className="text-center p-4 bg-surface-2 rounded-xl border border-border">
     <p className="text-2xl font-bold text-ink">{metrics.atRisk}</p>
     <p className="text-sm text-ink font-medium">At Risk</p>
     <p className="text-xs text-ink-3 mt-1">
      {total > 0 ? ((metrics.atRisk / total) * 100).toFixed(0) : 0}%
     </p>
    </div>
    <div className="text-center p-4 bg-err-soft rounded-xl border border-err">
     <p className="text-2xl font-bold text-err">{metrics.breached}</p>
     <p className="text-sm text-err font-medium">Breached</p>
     <p className="text-xs text-ink-3 mt-1">
      {total > 0 ? ((metrics.breached / total) * 100).toFixed(0) : 0}%
     </p>
    </div>
   </div>
   <div className="h-3 flex rounded-full overflow-hidden bg-surface-3">
    <div 
    ...
    />
    <div 
     className="bg-err transition-all" 
 ...
 return (
  <div className="space-y-6">
   ...
   <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
     <h2 className="text-xl font-bold text-ink">Ticket Dashboard</h2>
     <p className="text-sm text-ink-3">Overview of support tickets and team performance</p>
    </div>
    <div className="flex items-center gap-3">
     <input
      type="date"
      ...
      className="px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-border focus:border-transparent"
     ...
     <button
      ...
      className="p-2 bg-surface-3 text-ink rounded-lg hover:bg-surface-3 transition-colors"
   ...
   <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div className="lg:col-span-2 space-y-6">
     ...
     <div className="bg-surface rounded-xl border border-border p-6">
      <h3 className="text-lg font-semibold text-ink mb-4">Status Distribution</h3>
      <StatusProgress stats={stats.byStatus} total={stats.totalTickets} />
     </div>

     {/* Ticket Trend */}
     <div className="bg-surface rounded-xl border border-border p-6">
      <h3 className="text-lg font-semibold text-ink mb-4">7-Day Ticket Trend</h3>
     ...
     <div className="bg-surface rounded-xl border border-border p-6">
      ...
      {staffMetrics.length > 0 ? (
       ...
       <p className="text-center py-8 text-ink-3">No staff data available</p>
      )}
     </div>
    </div>
    ...
    <div className="space-y-6">
     ...
     <div className="bg-surface rounded-xl border border-border p-6">
      <h3 className="text-lg font-semibold text-ink mb-4">By Priority</h3>
     ...
     {/* SLA Metrics */}
     <div className="bg-surface rounded-xl border border-border p-6">
      <h3 className="text-lg font-semibold text-ink mb-4">SLA Performance</h3>
      <SLAMetrics metrics={stats.slaMetrics} />
     </div>

     {/* Response Metrics */}
     <div className="bg-surface rounded-xl border border-border p-6">
      <h3 className="text-lg font-semibold text-ink mb-4">Response Times</h3>
      <div className="space-y-4">
       <div className="flex items-center justify-between p-3 bg-surface-2 rounded-lg">
        <div className="flex items-center gap-3">
         ...
         <span className="text-sm text-ink-2">First Response</span>
        </div>
       ...
       <div className="flex items-center justify-between p-3 bg-surface-2 rounded-lg">
        <div className="flex items-center gap-3">
         ...
         <span className="text-sm text-ink-2">Avg Response</span>
        </div>
        <span className="text-lg font-bold text-ink">
       ...
       <div className="flex items-center justify-between p-3 bg-surface-2 rounded-lg">
        <div className="flex items-center gap-3">
         ...
         <span className="text-sm text-ink-2">Avg Resolution</span>
        </div>
        <span className="text-lg font-bold text-ink">
