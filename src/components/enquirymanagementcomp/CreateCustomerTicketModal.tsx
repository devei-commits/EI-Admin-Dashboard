...
const CreateCustomerTicketModal: React.FC<CreateCustomerTicketModalProps> = ({
 ...
 return (
  <ModalOverlay onClose={() => { if (!submitting) onClose(); }} z="z-50" dismissable={true} backdrop="default">
   <div role="dialog" aria-modal="true" aria-labelledby="create-customer-ticket-title" onClick={(e) => e.stopPropagation()} className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-surface border border-border shadow-xl">
    <div className="sticky top-0 z-10 flex items-center justify-between border-b border-hairline bg-surface px-5 py-4">
     <div>
      <h2 id="create-customer-ticket-title" className="text-lg font-semibold text-ink">New customer ticket</h2>
    ...
    <form onSubmit={(e) => void handleSubmit(e)} className="p-5 space-y-4">
     ...
     <div className="border-t border-hairline pt-3 space-y-3">
      <p className="text-xs font-semibold text-ink-3 uppercase">Customer</p>
      <div className="flex rounded-lg border border-border p-0.5 bg-surface-2 text-sm">
       <button
        type="button"
        onClick={() => {
         setCustomerMode('registered');
        ...
        className={`flex-1 rounded-md py-1.5 font-medium ${
         customerMode === 'registered' ? 'bg-surface shadow text-ink' : 'text-ink-2'
        }`}
       >
        Registered (website)
       </button>
       <button
        ...
        className={`flex-1 rounded-md py-1.5 font-medium ${
         customerMode === 'guest' ? 'bg-surface shadow text-ink' : 'text-ink-2'
        }`}
       >
        Guest
       </button>
      ...
      {customerMode === 'registered' ? (
       <div className="space-y-2">
        <label className="block text-sm font-medium text-ink-2">Find customer *</label>
        <input
         type="search"
         value={customerQuery}
        ...
        {!customerSearchLoading && customerQuery.trim().length >= 2 && customerResults.length === 0 && (
         <p className="text-xs text-ink-3">No matches. Try another email or name.</p>
        )}
        {customerResults.length > 0 && (
         <ul className="max-h-40 overflow-y-auto rounded-lg border border-border divide-y divide-hairline text-sm">
           ...
           <li key={row.userid}>
            ...
            >
             ...
             {row.vendor_client_code ? (
              <span className="block text-xs text-ink-4">{row.vendor_client_code}</span>
             ) : null}
            </button>
           </li>
          ))}
        ...
        {selectedCustomer && (
         <div className="rounded-lg border border-ok bg-ok-soft/60 px-3 py-2 text-sm">
          <p className="font-medium text-ok">Selected</p>
          <p className="text-ok">
          ...
          <button
           type="button"
           className="mt-1 text-xs text-ok underline"
           onClick={() => {
            setSelectedCustomer(null);
            setCustomerName('');
            setCustomerEmail('');
       ...
       <>
        ...
        <p className="text-xs text-warn bg-warn-soft border border-warn rounded-lg px-2 py-1.5">
     ...
     <div>
      ...
      <p className="text-xs text-ink-3 mt-1">Required. You can change assignee later; previous assignees are kept in history.</p>
     </div>
