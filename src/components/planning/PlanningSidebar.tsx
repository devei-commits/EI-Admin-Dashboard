...
import { ClipboardText, Package, Stack, CaretLeft, CaretRight, type Icon } from '@phosphor-icons/react';
...
export function PlanningSidebar({ activeView, onNavigate, counts }: PlanningSidebarProps): React.ReactElement {
  ...
  const handleSection = (view: PlanningView) => {
    onNavigate(view);
    setIsOpen(false);
  };
    ...
    <>
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 bg-surface border-b border-hairline z-50 flex items-center px-4 shadow-[var(--e1)]">
        ...
        <button
          ...
          aria-label={isOpen ? 'Close planning menu' : 'Open planning menu'}
        >
      ...
      >
        ...
        <nav className="flex-1 p-4 overflow-y-auto overflow-x-hidden">
          <p className={`text-xs font-semibold text-ink-4 uppercase tracking-wider px-4 mb-2 ${collapsed ? 'md:hidden' : ''}`}>Planning</p>
