...
export const GrievanceDetailPopup: React.FC<GrievanceDetailPopupProps> = ({ grievance, onClose, onUpdated }) => {
  ...
  return (
    <ProcModalShell
      ...
      subtitle={<span className="inline-flex items-center gap-2 text-xs text-ink-3">{row.client.displayCode} · {row.client.name} <StatusPill cfg={sevCfg} /></span>}
      width="max-w-2xl"
      onClose={onClose}
      footer={
        mode ? (
          <>
            ...
            {mode === 'respond' && <button onClick={() => { if (!text.trim()) { addToast('error', 'Write a response'); return; } run({ action: 'respond', text: text.trim() }, 'Response recorded'); }} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-brand px-4 py-2 text-sm font-bold text-white hover:bg-brand disabled:opacity-50">{busy ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />} Record response</button>}
            {mode === 'escalate' && <button onClick={() => { if (!dept && !toName.trim()) { addToast('error', 'Pick a dept or person'); return; } run({ action: 'escalate', dept, toName: toName.trim() || null, note: note.trim() || null }, 'Escalated'); }} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-warn px-4 py-2 text-sm font-bold text-white hover:bg-warn disabled:opacity-50">{busy ? <Loader2 size={14} className="animate-spin" /> : <ArrowUpRight size={14} />} Escalate</button>}
            ...
            {mode === 'resolve' && <button onClick={submitResolve} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-ok px-4 py-2 text-sm font-bold text-white hover:bg-ok disabled:opacity-50">{busy ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={14} />} Resolve</button>}
          </>
        ) : (
          <div className="flex w-full flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex gap-2">
              {!isClosed && <button onClick={openResolve} className="inline-flex items-center gap-1.5 rounded-lg bg-ok px-3 py-2 text-sm font-bold text-white hover:bg-ok"><CheckCircle2 size={14} /> Resolve</button>}
    ...
    >
      {mode === 'respond' || mode === 'rootcause' || mode === 'capa' ? (
        ...
        <div className="space-y-3">
          ...
          <label className="block"><span className={labelCls}>Person <span className="font-normal normal-case text-ink-4">(optional)</span></span><input value={toName} onChange={(e) => setToName(e.target.value)} className={inputCls} placeholder="Who should handle this?" /></label>
        ...
        <div className="space-y-3">
          <p className="rounded-lg border border-warn bg-warn-soft p-2 text-xs text-warn">Root cause and corrective action are mandatory before a grievance can be resolved.</p>
          ...
          <label className="flex items-center gap-2 text-sm text-ink-2"><input type="checkbox" checked={confirmed} onChange={(e) => setConfirmed(e.target.checked)} className="h-4 w-4 rounded border-border" /> Customer has confirmed the resolution</label>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          {row.escalation && (
            <ReadField label="Escalation"><div className="rounded-lg border border-warn bg-warn-soft p-3 text-sm text-warn"><b>{row.escalation.dept}</b>{row.escalation.toName ? ` · ${row.escalation.toName}` : ''}{row.escalation.note && <p className="mt-1 whitespace-pre-wrap text-xs text-warn">{row.escalation.note}</p>}</div></ReadField>
          )}
          {row.rootCause && <ReadField label="Root Cause"><p className="whitespace-pre-wrap rounded-lg border border-violet-200 bg-violet-50 p-3 text-sm text-ink-2">{row.rootCause}</p></ReadField>}
          {row.correctiveAction && <ReadField label="Corrective Action (CAPA)"><p className="whitespace-pre-wrap rounded-lg border border-cyan-200 bg-cyan-50 p-3 text-sm text-ink-2">{row.correctiveAction}</p></ReadField>}
          {row.response && <ReadField label={`Response${row.respondedAt ? ` · ${formatDMY(row.respondedAt)}` : ''}`}><p className="whitespace-pre-wrap rounded-lg border border-brand bg-brand-soft p-3 text-sm text-ink-2">{row.response}</p></ReadField>}
          {isClosed && <p className="text-xs text-ink-3">Customer confirmed: <b className={row.customerConfirmed ? 'text-ok' : 'text-ink-3'}>{row.customerConfirmed ? 'Yes' : 'No'}</b></p>}
        </div>
      )}
    </ProcModalShell>
  );
