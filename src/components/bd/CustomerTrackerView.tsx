    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-surface px-4 py-2.5 shadow-sm">
        <div className="flex items-center gap-1.5 text-xs text-ink-2">
          ...
          <span><b className="text-ink">Customer Tracker</b> · {rows.length} clients · {openOrders} open orders
          {totalReceivable > 0 && <span className="ml-2 text-ink-3">· receivable {formatINRCompact(totalReceivable)}</span>}</span>
        </div>
        <button onClick={onRefresh} title="Refresh" aria-label="Refresh" className="rounded-lg border border-border p-2 text-ink-3 hover:bg-surface-3">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-surface-2/70 p-3">
        <div className="relative min-w-[200px] flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-border bg-surface py-2 pl-9 pr-3 text-sm focus:ring-2 focus:ring-blue-500"
          />
        </div>
        <select value={tierFilter} onChange={(e) => setTierFilter(e.target.value as typeof tierFilter)}
          aria-label="Filter by tier"
          className="rounded-lg border border-border bg-surface px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500">
      ...
      {/* Table */}
      {loading ? (
        <div className="rounded-xl border border-border bg-surface p-4"><TableSkeleton rows={6} cols={13} /></div>
      ) : error ? (
        <div className="rounded-xl border border-border bg-surface"><ErrorState message={error} onRetry={onRefresh} /></div>
      ) : filtered.length === 0 ? (
        <div className="rounded-xl border border-border bg-surface"><EmptyState icon={<Users />} title={rows.length === 0 ? 'No clients found.' : 'No clients match your filters.'} /></div>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead className="sticky top-0 z-20 [&_th]:bg-surface-2">
              <tr className="border-b border-border bg-surface-2">
                {HEADERS.map((h) => (
                  <th scope="col" key={h} className="whitespace-nowrap px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-ink-3">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-hairline">
              {filtered.map((r) => {
                ...
                return (
                  <tr key={r.id} className="transition-colors hover:bg-brand-soft/30">
                    {/* Customer */}
                    <td className="px-3 py-2.5">
                      <button onClick={() => onOpenDetail(r.code)} className="text-left">
                        <span className="block font-mono text-[11px] font-semibold text-brand hover:underline decoration-dotted">{r.displayCode}</span>
                        <span className="block max-w-[180px] truncate text-xs font-semibold text-ink" title={r.name}>{r.name}</span>
                        <span className="mt-0.5 block"><TierPill tier={r.tier} /></span>
                      </button>
                    </td>
                    {/* Location */}
                    <td className="max-w-[130px] px-3 py-2.5">
                      <p className="truncate text-xs text-ink-2">{r.city || r.location || '—'}</p>
                      {r.country && <p className="truncate text-[10px] text-ink-4">{r.country}</p>}
                    </td>
                    {/* BD POC */}
                    <td className="max-w-[120px] px-3 py-2.5"><p className="truncate text-xs text-ink-2">{r.bdPoc.name || '—'}</p></td>
                    {/* Receivable */}
                    <td className={`whitespace-nowrap px-3 py-2.5 text-xs tabular-nums ${overdue ? 'font-bold text-err' : 'text-ink-2'}`}>
                      {formatINRCompact(r.receivable)}
                      {overdue && <span className="ml-1 text-[9px]">overdue</span>}
                    </td>
                    {/* Advances */}
                    <td className="whitespace-nowrap px-3 py-2.5 text-xs tabular-nums text-ok">{formatINRCompact(r.advances)}</td>
                    {/* Status */}
                    <td className="whitespace-nowrap px-3 py-2.5"><LifecyclePill lifecycle={r.lifecycle} /></td>
                    {/* Orders */}
                    <td className="whitespace-nowrap px-3 py-2.5 text-xs tabular-nums text-ink-2">
                      <b className={r.orders.open > 0 ? 'text-brand' : 'text-ink-4'}>{r.orders.open}</b>
                      <span className="text-ink-4"> / {r.orders.ytd} YTD</span>
                    </td>
                    {/* PIS */}
                    <td className="whitespace-nowrap px-3 py-2.5 text-xs tabular-nums text-ink-2">
                      <b className={r.pis.inFlight > 0 ? 'text-violet-600' : 'text-ink-4'}>{r.pis.inFlight}</b>
                      <span className="text-ink-4"> / {r.pis.total}</span>
                    </td>
                    {/* Queries */}
                    <td className="whitespace-nowrap px-3 py-2.5 text-xs tabular-nums">
                      <span className={r.queries.open > 0 ? 'font-semibold text-warn' : 'text-ink-4'}>{r.queries.open}</span>
                      {r.queries.breached > 0 && <span className="ml-1 inline-flex items-center font-bold text-err"><Flag size={9} className="mr-0.5" />{r.queries.breached}</span>}
                    </td>
                    {/* Next Meeting */}
                    <td className="whitespace-nowrap px-3 py-2.5 text-xs text-ink-2">{r.nextMeeting?.date ? formatDMY(r.nextMeeting.date) : '—'}</td>
                    {/* Grievances */}
                    <td className="whitespace-nowrap px-3 py-2.5 text-xs tabular-nums">
                      <span className={r.grievances.open > 0 ? 'font-semibold text-err' : 'text-ink-4'}>{r.grievances.open}</span>
                      {r.grievances.escalated > 0 && <span className="ml-1 inline-flex items-center text-[10px] text-warn"><ArrowUp size={9} className="mr-0.5" />{r.grievances.escalated}</span>}
                    </td>
                    {/* History */}
                    <td className="px-3 py-2.5">
                      <button onClick={() => onOpenTimeline(r.code)} title="History & comments" aria-label="History & comments"
                        className="rounded-md border border-border p-1.5 text-ink-3 hover:bg-surface-3 hover:text-brand">
                        <BookText size={14} />
                      </button>
                    </td>
                    {/* Actions */}
                    <td className="whitespace-nowrap px-3 py-2.5">
                      <div className="flex items-center gap-1">
                        <button onClick={() => onAction('meeting', r)} title="Add meeting" aria-label="Add meeting"
                          className="rounded-md border border-brand p-1.5 text-brand hover:bg-brand-soft"><CalendarPlus size={13} /></button>
                        <button onClick={() => onAction('query', r)} title="Raise query" aria-label="Raise query"
                          className="rounded-md border border-warn p-1.5 text-warn hover:bg-warn-soft"><HelpCircle size={13} /></button>
                        <button onClick={() => onAction('grievance', r)} title="Log grievance" aria-label="Log grievance"
                          className="rounded-md border border-err p-1.5 text-err hover:bg-err-soft"><Flag size={13} /></button>
                      </div>
                    </td>
                  </tr>
                );
