...
const EditRoleFullPage: React.FC<EditRoleFullPageProps> = ({ role, users, onClose, onSave }) => {
 ...
 const getLevelColor = (level: string) => {
  switch (level) {
   ...
   case 'manager':
    return 'bg-gray-50 text-slate-900 border border-gray-100';
   case 'staff':
          ...
          <button
           ...
           className={`w-full text-left px-2 lg:px-3 py-1.5 lg:py-2 rounded-lg text-xs lg:text-sm transition-all duration-200 ${
            editedRole.roleName === roleName
             ? 'bg-gray-100 text-amber-800 font-medium'
      ...
      <div className="max-w-6xl mx-auto space-y-6">
       ...
       <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
        <h3 className="text-base sm:text-lg font-semibold text-gray-800 mb-4 flex items-center">
         <svg className="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-slate-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
         </svg>
         Role Settings
        ...
        <div className="md:hidden mb-4 p-3 bg-gray-50 border border-gray-100 rounded-lg">
         <label className="block text-xs font-medium text-slate-900 mb-2">Quick Role Select</label>
         <select
          ...
          className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-800 bg-white"
         >
          {Object.entries(roleHierarchy).map(([level, roles]) => (
