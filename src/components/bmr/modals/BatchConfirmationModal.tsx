...
const BatchConfirmationModal: React.FC<BatchConfirmationModalProps> = ({
  ...
  const [selectedGapMaterial, setSelectedGapMaterial] = useState<StockMaterialItem | null>(null);
  ...
  const _totalGapQuantity = materials.reduce((sum, m) => sum + (m.gap || 0), 0);
  const materialsWithGaps = materials.filter(m => m.status === 'Gap');

  const handleOpenUpdateStockModal = (material: StockMaterialItem | null = null) => {
    if (material) {
  ...
  const handleApplyStockUpdate = (updatedMaterial: StockMaterialItem) => {
