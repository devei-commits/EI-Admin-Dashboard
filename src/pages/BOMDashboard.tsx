...
const BOMDashboard: React.FC = () => {
  ...
  const statCardData = [
    { label: 'TOTAL PRODUCTS', value: list.length, sub: 'Registered PR masters', accent: 'border-l-brand', num: 'text-brand' },
    { label: 'ACTIVE', value: list.filter((p) => p.status === 'Active').length, sub: 'Approved masters', accent: 'border-l-ok', num: 'text-ok' },
    { label: 'CATEGORIES', value: [...new Set(list.map((p) => p.category).filter(Boolean))].length, sub: 'Product categories', accent: 'border-l-warn', num: 'text-warn' },
    { label: 'RM INGREDIENTS', value: list.reduce((sum, p) => sum + (p.rm_ingredients_count ?? 0), 0), sub: 'Total in formulas', accent: 'border-l-brand', num: 'text-brand' },
    { label: 'PM COMPONENTS', value: list.reduce((sum, p) => sum + (p.pack_items_count ?? 0), 0), sub: 'Total pack items', accent: 'border-l-err', num: 'text-err' },
  ];
  ...
  return (
    <div className="min-h-screen bg-canvas">
        ...
        <div className="bg-surface rounded-2xl border border-hairline shadow-sm overflow-hidden hover:shadow-lg transition-shadow duration-300">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 px-6 py-5 border-b border-hairline bg-surface-2">
            <div className="flex items-center gap-2 min-w-0">
              <span className="text-sm font-semibold text-ink">Products Master</span>
              <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-brand-soft text-brand border border-brand-soft">{pagedFilteredList.length} / {totalFiltered}</span>
            </div>
                ...
                <>
                  ...
                  {formulaRmExcelUploading && formulaBomUploadPercent != null && (
                    <div className="flex items-center gap-2 min-w-[10rem]">
                      <div className="flex-1 h-2 bg-surface-3 rounded-full overflow-hidden max-w-[9rem]">
                        <div
                          className="h-full bg-brand transition-[width] duration-150 ease-out"
                          style={{ width: `${formulaBomUploadPercent}%` }}
                        />
                      </div>
                      <span className="text-xs text-ink-3 tabular-nums w-9">{formulaBomUploadPercent}%</span>
                    </div>
                  )}
                </>
              )}
                ...
                <thead className="bg-surface-3 border-b border-border">
                  <tr>
                    ...
                    <SortableTableTh
                      ...
                      accent="cyan"
                    />
                    <th scope="col" className="px-4 py-3 text-left text-xs font-semibold text-ink-3 uppercase tracking-wide">RM approval</th>
                    <th scope="col" className="px-4 py-3 text-left text-xs font-semibold text-ink-3 uppercase tracking-wide">PM approval</th>
                    <th scope="col" className="px-4 py-3 text-left text-xs font-semibold text-ink-3 uppercase tracking-wide">RM assign</th>
                    <th scope="col" className="px-4 py-3 text-left text-xs font-semibold text-ink-3 uppercase tracking-wide">Pack assign</th>
                    <SortableTableTh
                      label="Open SOs"
                      column="openSos"
                      sortColumn={sortColumn}
                      ...
                      accent="cyan"
                    />
                    <th scope="col" className="px-4 py-3 text-left text-xs font-semibold text-ink-3 uppercase tracking-wide">Logs</th>
                ...
                <tbody className="divide-y divide-border">
                  {filteredList.length === 0 ? (
                    ...
                    pagedFilteredList.map((p, idx) => (
                      <tr key={p.product_id} className="hover:bg-surface-3 transition-colors cursor-pointer" onClick={() => handleViewItem(p)}>
                        <td className="px-4 py-3 text-sm text-ink-3">{startIndex + idx + 1}</td>
                        <td className="px-4 py-3 text-sm font-mono font-semibold text-ink">{p.product_code || '—'}</td>
                        <td className="px-4 py-3 text-sm">
                          {p.pr_record_type === 'temporary' ? (
                            <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-warn-soft text-warn border border-[color:var(--st-amber-fg)]/30">
                              Temporary
                            </span>
                          ) : p.pr_record_type === 'permanent' ? (
                            <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-surface-3 text-ink border border-border">
                              Permanent
                            </span>
                          ) : (
                        ...
                        <td className="px-4 py-3">
                          <div>
                            <p className="text-sm font-semibold text-ink">{p.product_name || 'Product'}</p>
                            <p className="text-xs text-ink-3">SKU: {(p as unknown as { zoho_sku_code?: string; product_sku?: string }).zoho_sku_code ?? (p as unknown as { product_sku?: string }).product_sku ?? '—'}</p>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-sm text-ink-3">{p.category || '—'}</td>
                        <td className="px-4 py-3 text-sm text-ink-3">{p.pr_sub_category || '—'}</td>
                        <td className="px-4 py-3 text-sm text-ink-3">{p.form ?? '—'}</td>
                        <td className="px-4 py-3 text-sm font-mono text-ink-3">
                          {formatSkuBomLimitAsPack(p.skuBomLimitQty, p.skuBomLimitUom)}
                        </td>
                        <td className="px-4 py-3 text-sm font-mono text-ink-3">{p.batch_size_kg ?? '—'}</td>
                        <td className="px-4 py-3 text-sm font-mono text-ink-3">{p.shelf_life_months != null ? `${p.shelf_life_months}M` : '—'}</td>
                        <td className="px-4 py-3 text-sm font-mono font-semibold text-brand text-center">{p.rm_ingredients_count ?? 0}</td>
                        <td className="px-4 py-3 text-sm font-mono font-semibold text-warn text-center">{p.pack_items_count ?? 0}</td>
                        ...
                        <td className="px-4 py-3 text-center">
                          {(p.open_sos_count ?? 0) > 0 ? (
                            <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-bold font-mono bg-brand-soft text-brand border border-brand-soft">{p.open_sos_count}</span>
                          ) : (
                            '—'
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3 text-right whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                          <button
                            ...
                            className="text-xs font-semibold text-brand hover:text-brand hover:underline mr-2"
                          >
                            Edit
                          </button>
                          <button
                            ...
                            className="text-xs font-semibold text-err hover:text-err hover:underline"
                          >
                            Delete
                          </button>
                        </td>
      ...
      {isPanelOpen && (
        <div className="fixed inset-y-0 right-0 w-full max-w-2xl bg-surface shadow-2xl z-50 flex flex-col border-l border-border">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-surface-3">
            <div>
              <span className="text-sm font-mono text-ink-3">{selectedProduct?.product_code ?? '—'}</span>
              <h2 className="text-lg font-bold text-ink">{selectedProduct?.product_name ?? 'Product'}</h2>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={handleClosePanel} className="p-2 rounded-lg hover:bg-surface-3 text-ink-3">X</button>
            </div>
          </div>
          {detailLoading ? (
            ...
            <>
              <div className="flex gap-1 px-4 py-2 border-b border-hairline bg-surface-2">
                {['Overview', 'Formula BOM', 'SKU BOM', 'Pack BOM', 'Process', 'Specs & Stability'].map((label, i) => (
                  <button
                    ...
                    className={`px-3 py-2 text-sm font-medium rounded-lg transition-colors ${panelTab === i ? 'bg-brand text-white' : 'text-ink-3 hover:bg-surface-3'}`}
                  >
                    {label}
                  </button>
                ))}
              ...
              <div className="flex-1 overflow-y-auto p-6">
                {panelTab === 0 && displayProduct && (
                  <div className="space-y-6">
                    <div>
                      <div className="text-xs font-semibold text-ink-3 uppercase mb-2">Identity</div>
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div className="col-span-2">
                          <span className="text-ink-3">Internal PR code</span>
                          {isEditMode ? (
                            <input
                              value={displayProduct.product_code ?? ''}
                              onChange={(e) => updateDraft({ product_code: e.target.value })}
                              className="mt-1 w-full px-2 py-1.5 border border-border rounded font-mono text-sm text-ink"
                            />
                          ) : (
                            <div className="font-mono mt-1 font-semibold text-ink">{displayProduct.product_code ?? '—'}</div>
                          )}
                        </div>
                        <div className="col-span-2">
                          <span className="text-ink-3">PR record type</span>
                          {isEditMode ? (
                            <div className="mt-2 flex flex-wrap gap-4 text-sm text-ink">
                              <label className="inline-flex items-center gap-2 cursor-pointer">
                                ...
                                />
                                Permanent <span className="font-mono text-xs text-ink-3">(PR…)</span>
                              </label>
                              <label className="inline-flex items-center gap-2 cursor-pointer">
                                <input
                                  type="radio"
                                ...
                                />
                                Temporary <span className="font-mono text-xs text-ink-3">(TPR…)</span>
                              </label>
                              <label className="inline-flex items-center gap-2 cursor-pointer">
                                <input
                                  type="radio"
                            ...
                            <div className="mt-1">
                              {displayProduct.pr_record_type === 'temporary' ? (
                                <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-warn-soft text-warn">Temporary</span>
                              ) : displayProduct.pr_record_type === 'permanent' ? (
                                <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-surface-3 text-ink">Permanent</span>
                              ) : (
                                <span className="text-ink-3">Legacy / unspecified</span>
                              )}
                            </div>
                          )}
                        </div>
                        <div><span className="text-ink-3">Category</span>
                          {isEditMode ? <input value={displayProduct.category ?? ''} onChange={(e) => updateDraft({ category: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded text-ink" /> : <div className="font-medium">{displayProduct.category ?? '—'}</div>}
                        </div>
                        <div><span className="text-ink-3">Status</span>
                          {isEditMode ? (
                            <select value={displayProduct.status ?? ''} onChange={(e) => updateDraft({ status: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded text-ink">
                              {STATUS_OPTIONS.map((s) => <option key={s} value={s}>{s}</option>)}
                            </select>
                          ) : <div><span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-ok-soft text-ok">{displayProduct.status ?? '—'}</span></div>}
                        </div>
                        <div><span className="text-ink-3">Form</span>
                          {isEditMode ? <input value={displayProduct.form ?? ''} onChange={(e) => updateDraft({ form: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded font-mono" /> : <div className="font-mono">{displayProduct.form ?? '—'}</div>}
                        </div>
                        <div><span className="text-ink-3">Pack size (SKU BOM)</span>
                        ...
                        <div><span className="text-ink-3">SKU Code</span>
                          {isEditMode ? (
                            <input
                              value={
                              ...
                              className="mt-1 w-full px-2 py-1.5 border border-border rounded font-mono"
                            />
                          ) : (
                        ...
                        <div><span className="text-ink-3">License / CML</span>
                          {isEditMode ? <input value={displayProduct.license_cml ?? ''} onChange={(e) => updateDraft({ license_cml: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded font-mono" /> : <div className="font-mono">{displayProduct.license_cml ?? '—'}</div>}
                        </div>
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-semibold text-ink-3 uppercase mb-2">Commercials</div>
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div><span className="text-ink-3">Version</span>
                          {isEditMode ? <input value={displayProduct.version ?? ''} onChange={(e) => updateDraft({ version: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded font-mono" /> : <div className="font-mono">{displayProduct.version ?? '—'}</div>}
                        </div>
                        <div><span className="text-ink-3">Batch Size (KG)</span>
                          {isEditMode ? <input type="number" value={displayProduct.batch_size_kg ?? ''} onChange={(e) => updateDraft({ batch_size_kg: e.target.value === '' ? undefined : Number(e.target.value) })} className="mt-1 w-full px-2 py-1.5 border border-border rounded font-mono" /> : <div className="font-mono">{displayProduct.batch_size_kg != null ? `${displayProduct.batch_size_kg} KG` : '—'}</div>}
                        </div>
                        <div><span className="text-ink-3">Yield %</span>
                          {isEditMode ? <input type="number" step="0.01" value={displayProduct.theoretical_yield_pct ?? ''} onChange={(e) => updateDraft({ theoretical_yield_pct: e.target.value === '' ? undefined : Number(e.target.value) })} className="mt-1 w-full px-2 py-1.5 border border-border rounded font-mono" /> : <div className="font-mono">{displayProduct.theoretical_yield_pct != null ? `${displayProduct.theoretical_yield_pct}%` : '—'}</div>}
                        </div>
                        <div><span className="text-ink-3">Shelf Life (months)</span>
                          {isEditMode ? <input type="number" value={displayProduct.shelf_life_months ?? ''} onChange={(e) => updateDraft({ shelf_life_months: e.target.value === '' ? undefined : Number(e.target.value) })} className="mt-1 w-full px-2 py-1.5 border border-border rounded" /> : <div>{displayProduct.shelf_life_months != null ? `${displayProduct.shelf_life_months} months` : '—'}</div>}
                        </div>
                        <div><span className="text-ink-3">PAO (months)</span>
                          {isEditMode ? <input type="number" value={displayProduct.pao_months ?? ''} onChange={(e) => updateDraft({ pao_months: e.target.value === '' ? undefined : Number(e.target.value) })} className="mt-1 w-full px-2 py-1.5 border border-border rounded" /> : <div>{displayProduct.pao_months != null ? `${displayProduct.pao_months} months` : '—'}</div>}
                        </div>
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-semibold text-ink-3 uppercase mb-2">Manufacturing</div>
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div className="col-span-2"><span className="text-ink-3">Location</span>
                          {isEditMode ? <input value={displayProduct.manufacturing_location ?? ''} onChange={(e) => updateDraft({ manufacturing_location: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded" /> : <div>{displayProduct.manufacturing_location ?? '—'}</div>}
                        </div>
                        <div className="col-span-2"><span className="text-ink-3">Equipment</span>
                          {isEditMode ? <input value={displayProduct.equipment_vessel ?? ''} onChange={(e) => updateDraft({ equipment_vessel: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded" /> : <div>{displayProduct.equipment_vessel ?? '—'}</div>}
                        </div>
                        <div className="col-span-2"><span className="text-ink-3">Storage</span>
                          {isEditMode ? <input value={displayProduct.storage_conditions ?? ''} onChange={(e) => updateDraft({ storage_conditions: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded" /> : <div>{displayProduct.storage_conditions ?? '—'}</div>}
                        </div>
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-semibold text-ink-3 uppercase mb-2">Claims</div>
                      {isEditMode ? <textarea value={displayProduct.approved_claims ?? ''} onChange={(e) => updateDraft({ approved_claims: e.target.value })} rows={3} className="w-full px-2 py-1.5 border border-border rounded text-sm" /> : <div className="p-3 bg-surface-3 rounded-lg text-sm">{displayProduct.approved_claims ?? '—'}</div>}
                    </div>
                    <div>
                      <div className="text-xs font-semibold text-ink-3 uppercase mb-2">Open Sales Orders</div>
                      <div className="space-y-2">
                        {(selectedProduct?.openSalesOrders ?? []).length === 0 ? (
                          ...
                          (selectedProduct?.openSalesOrders ?? []).map((so) => (
                            <div key={so.order_id} className="flex items-center gap-3 p-3 bg-surface-3 border border-hairline rounded-lg">
                              <span className="font-mono text-xs font-bold text-brand">{so.order_id}</span>
                              <span className="flex-1 text-sm text-ink-2">{so.customer_name}</span>
                              <span className="font-mono text-sm font-bold text-warn">{Number(so.quantity).toLocaleString()} units</span>
                              <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-ok-soft text-ok">{so.status}</span>
                            </div>
                          ))
                        )}
                      </div>
                    ...
                    <div className="space-y-4">
                      {isEditMode && (
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                          <button type="button" onClick={addFormulaPhase} className="inline-flex items-center gap-1 px-2 py-1.5 text-xs font-medium bg-brand text-white rounded hover:bg-brand-press">
                            <Plus className="w-3.5 h-3.5" /> Add phase
                          </button>
                        </div>
                      )}
                      {formulaList.map((phase, phaseIdx) => (
                        <div key={phaseIdx} className="border border-border rounded-lg p-3">
                          <div className="flex items-center gap-2 mb-2 flex-wrap">
                            {isEditMode ? (
                              <input value={phase.phase} onChange={(e) => updateFormulaPhaseName(phaseIdx, e.target.value)} className="px-2 py-1 rounded text-xs font-semibold bg-brand-soft border border-brand-soft w-40" placeholder="Phase name" />
                            ) : (
                              <span className="px-2 py-0.5 rounded text-xs font-semibold bg-brand-soft text-brand">{phase.phase}</span>
                            )}
                            <span className="text-xs text-ink-3">{phase.ingredients.length} ingredient(s)</span>
                            {isEditMode && (
                              <>
                                <button type="button" onClick={() => addFormulaIngredient(phaseIdx)} className="text-xs text-brand hover:underline">+ Ingredient</button>
                                <button type="button" onClick={() => removeFormulaPhase(phaseIdx)} className="text-err hover:text-err p-0.5" title="Remove phase"><Trash2 className="w-3.5 h-3.5" /></button>
                              </>
                            )}
                          </div>
                          <div className="overflow-x-auto">
                              ...
                              <tbody>
                                {phase.ingredients.map((ing, i) => {
                                  ...
                                  <tr key={i} className="border-t border-hairline">
                                    <td className="p-2 text-ink-4 font-mono">{i + 1}</td>
                                    {isEditMode ? (
                                      <>
                                        <td className="p-2">
                                          <select
                                            ...
                                            className="w-full px-1 py-1 border rounded text-xs bg-surface"
                                          >
                                          ...
                                          <>
                                            <td className="p-2" colSpan={2}>
                                              <select
                                                ...
                                                className="w-full px-2 py-1 border rounded text-xs bg-surface"
                                              >
                                        ...
                                        <td className="p-2"><button type="button" onClick={() => removeFormulaIngredient(phaseIdx, i)} className="text-err hover:text-err"><Trash2 className="w-3.5 h-3.5" /></button></td>
                                      </>
                                    ) : (
                                      <>
                                        <td className="p-2">
                                          {isGroupLine ? (
                                            <span className="text-[10px] font-semibold uppercase text-brand bg-brand-soft px-1.5 py-0.5 rounded">Group</span>
                                          ) : (
                                            <span className="text-[10px] font-semibold uppercase text-ink-3">RM</span>
                                          )}
                                        </td>
                                        <td className="p-2 font-medium">
                                          {ing.inci_name}
                                          {isGroupLine && ing.item_group_name ? (
                                            <span className="block text-[10px] text-brand font-normal">
                                              Swap among group members at Planning BOM confirm
                                            </span>
                                          ) : null}
                                        </td>
                                        <td className="p-2 font-mono text-xs text-brand">{ing.rm_code}</td>
                                        <td className="p-2 text-right font-mono">{ing.pct_w_w}</td>
                                        <td className="p-2 text-ink-3">{ing.uom}</td>
                                      </>
                                    )}
                                  </tr>
                                  );
                      ...
                      {formulaList.length === 0 && <p className="text-ink-3 text-sm">No formula BOM data. {isEditMode && 'Add a phase above.'}</p>}
                    </div>
                  );
                })()}
                {panelTab === 2 && (selectedProduct || editDraft) && (() => {
                  ...
                  return (
                    <div className="space-y-4">
                      ...
                      {hasNet ? (
                        <div className="p-3 bg-brand-soft border border-brand-soft rounded-lg space-y-1">
                          <p className="text-[10px] font-semibold text-brand uppercase">Net per 1 product unit</p>
                          <p className="text-lg font-mono font-bold text-brand">
                            {limQ}{' '}
                            <span className="text-base font-semibold text-brand">{limU}</span>
                        ...
                        <div className="flex flex-wrap items-end gap-3 p-3 bg-warn-soft border border-[color:var(--st-amber-fg)]/30 rounded-lg">
                          <div>
                            <label className="block text-[10px] font-semibold text-warn uppercase mb-1">Net / unit qty (manual)</label>
                            <input
                              type="number"
                              step="0.0001"
                              value={limQ ?? ''}
                              ...
                              className="w-28 px-2 py-1.5 border border-[color:var(--st-amber-fg)]/30 rounded text-sm"
                            />
                          </div>
                          <div>
                            <label className="block text-[10px] font-semibold text-warn uppercase mb-1">UOM</label>
                            <select
                              value={limU || 'GM'}
                              onChange={(e) => updateDraft({ skuBomLimitUom: e.target.value })}
                              className="px-2 py-1.5 border border-[color:var(--st-amber-fg)]/30 rounded text-sm"
                            >
                        ...
                        <p className="text-sm font-mono text-brand">
                          ...
                          <span className="block text-xs font-normal text-ink-3 mt-1">Set net per unit (qty + UOM) for pack size on sale orders.</span>
                        </p>
                      )}
                      {canEdit && (
                        <div className="p-3 rounded-lg border border-brand-soft bg-brand-soft space-y-2">
                          <div className="flex items-start justify-between gap-3 flex-wrap">
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold border border-brand-soft bg-surface text-brand hover:bg-brand-press shrink-0"
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="bg-surface-3">
                              ...
                              <th scope="col" className="text-right p-2">Required / unit (Std UoM)</th>
                            </tr>
                          </thead>
                          <tbody>
                            {skuList.map((row, i) => {
                              ...
                              return (
                                <tr key={i} className="border-t border-hairline">
                                  ...
                                  <td className="p-2 font-mono text-xs text-brand">{row.rm_code}</td>
                ...
                {panelTab === 3 && (selectedProduct || editDraft) && (() => {
                  ...
                  return (
                    <div>
                      ...
                      {isEditMode && (
                        <button type="button" onClick={addPackRow} className="inline-flex items-center gap-1 px-2 py-1.5 text-xs font-medium bg-brand text-white rounded hover:bg-brand-press mb-3">
                                  ...
                                  <>
                                    ...
                                    <td className="p-2">
                                      <select
                                        ...
                                        className="w-full min-w-[7rem] px-2 py-1 border rounded text-xs disabled:bg-surface-3"
                                      >
                                    ...
                                    <td className="p-2"><button type="button" onClick={() => removePackRow(i)} className="text-err hover:text-err"><Trash2 className="w-3.5 h-3.5" /></button></td>
                                  </>
                                ) : (
                                  <>
                                    <td className="p-2 font-medium">{row.pm_description}</td>
                                    <td className="p-2">
                                      <Link to={`/packaging?pm=${encodeURIComponent(row.pm_code)}`} className="font-mono text-xs text-warn hover:text-warn underline" title="Open in Pack Materials to edit; changes apply everywhere">{row.pm_code}</Link>
                                    </td>
                                    <td className="p-2 text-xs text-ink-2">{categoryLabel}</td>
                                    <td className="p-2 text-xs text-ink-2">{row.pm_sub_category || '—'}</td>
                                    <td className="p-2 text-xs text-ink-2">{row.pm_sub_sub_category || '—'}</td>
                                    <td className="p-2"><span className="text-xs font-semibold px-1.5 py-0.5 rounded-full bg-ok-soft text-ok">{row.pack_type}</span></td>
                                    <td className="p-2 text-right font-mono font-bold text-brand">{row.qty_per_unit}</td>
                                    <td className="p-2 text-ink-3">{toPmDisplayUnit(row.uom)}</td>
                                  </>
                                )}
                              </tr>
                            );
                      ...
                      <p className="text-xs text-ink-3 mt-2">{packList.length} packaging component(s)</p>
                      {packList.length === 0 && <p className="text-ink-3 text-sm mt-2">No pack BOM data. {isEditMode && 'Add a row above.'}</p>}
                    </div>
                  );
                })()}
                {panelTab === 4 && (selectedProduct || editDraft) && (() => {
                  ...
                  return (
                    <div className="space-y-2">
                      {isEditMode && (
                        <button type="button" onClick={addProcessStep} className="inline-flex items-center gap-1 px-2 py-1.5 text-xs font-medium bg-brand text-white rounded hover:bg-brand-press mb-2">
                          <Plus className="w-3.5 h-3.5" /> Add step
                        </button>
                      )}
                      {stepsList.map((step, i) => (
                        <div key={i} className="flex items-start gap-3 p-3 border border-hairline rounded-lg">
                          <span className="shrink-0 w-8 h-8 rounded-full bg-brand-soft text-brand font-bold text-sm flex items-center justify-center">{i + 1}</span>
                          {isEditMode ? (
                            <>
                              ...
                              <button type="button" onClick={() => removeProcessStep(i)} className="shrink-0 text-err hover:text-err p-1" title="Remove step"><Trash2 className="w-4 h-4" /></button>
                            </>
                          ) : (
                            <>
                              <div className="flex-1 min-w-0"><p className="text-sm text-ink">{step.description}</p></div>
                              <span className="shrink-0 text-sm font-mono text-ink-3">{step.duration_minutes} min</span>
                            </>
                          )}
                        </div>
                      ))}
                      {stepsList.length === 0 && <p className="text-ink-3 text-sm">No process steps. {isEditMode && 'Add a step above.'}</p>}
                      {stepsList.length > 0 && <p className="text-xs text-ink-3 text-right">Total steps: {stepsList.length}</p>}
                    </div>
                  );
                })()}
                {panelTab === 5 && displayProduct && (
                  <div className="space-y-6">
                    <div>
                      <div className="text-xs font-semibold text-ink-3 uppercase mb-2">FP Specifications</div>
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div><span className="text-ink-3">pH Range</span>
                          {isEditMode ? <input value={displayProduct.ph_range ?? ''} onChange={(e) => updateDraft({ ph_range: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded font-mono" /> : <div className="font-mono">{displayProduct.ph_range ?? '—'}</div>}
                        </div>
                        <div><span className="text-ink-3">Viscosity (cPs)</span>
                          {isEditMode ? <input value={displayProduct.viscosity_range ?? ''} onChange={(e) => updateDraft({ viscosity_range: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded font-mono" /> : <div className="font-mono">{displayProduct.viscosity_range ?? '—'}</div>}
                        </div>
                        <div><span className="text-ink-3">SPF / PA</span>
                          {isEditMode ? <input value={displayProduct.spf_pa_rating ?? ''} onChange={(e) => updateDraft({ spf_pa_rating: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded" /> : <div>{displayProduct.spf_pa_rating ?? '—'}</div>}
                        </div>
                        <div><span className="text-ink-3">Appearance</span>
                          {isEditMode ? <input value={displayProduct.appearance ?? ''} onChange={(e) => updateDraft({ appearance: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded" /> : <div>{displayProduct.appearance ?? '—'}</div>}
                        </div>
                        <div><span className="text-ink-3">Odour</span>
                          {isEditMode ? <input value={displayProduct.odour ?? ''} onChange={(e) => updateDraft({ odour: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded" /> : <div>{displayProduct.odour ?? '—'}</div>}
                        </div>
                        <div><span className="text-ink-3">Fill Weight</span>
                          {isEditMode ? <input value={displayProduct.fill_weight_spec ?? ''} onChange={(e) => updateDraft({ fill_weight_spec: e.target.value })} className="mt-1 w-full px-2 py-1.5 border border-border rounded font-mono" /> : <div className="font-mono">{displayProduct.fill_weight_spec ?? '—'}</div>}
                        </div>
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-semibold text-ink-3 uppercase mb-2">Stability</div>
                      {isEditMode ? <textarea value={displayProduct.stability_summary ?? ''} onChange={(e) => updateDraft({ stability_summary: e.target.value })} rows={3} className="w-full px-2 py-1.5 border border-border rounded text-sm text-ink" /> : <div className="p-3 bg-ok-soft border border-[color:var(--st-green-fg)]/30 rounded-lg text-sm text-ok">{displayProduct.stability_summary ?? '—'}</div>}
                    </div>
                  </div>
                )}
              </div>
              <div className="px-6 py-4 border-t border-border flex gap-2">
                {isEditMode ? (
                  <>
                    <button onClick={cancelEdit} className="px-4 py-2 text-sm font-medium text-ink-2 bg-surface border border-border rounded-lg hover:bg-surface-3">Cancel</button>
                    <button onClick={handleSave} disabled={saving} className="px-4 py-2 text-sm font-medium text-white bg-ok rounded-lg hover:bg-ok disabled:opacity-50">{saving ? 'Saving…' : 'Save'}</button>
                  </>
                ) : (
                  <>
                    <button onClick={handleClosePanel} className="px-4 py-2 text-sm font-medium text-ink-2 bg-surface border border-border rounded-lg hover:bg-surface-3">Close</button>
                    {canEdit && selectedProduct && (
                      <button
                        onClick={() => { setBomEditPopupId(String(selectedProduct.product_id)); }}
                        className="px-4 py-2 text-sm font-medium text-white bg-brand rounded-lg hover:bg-brand-press"
                      >
                        Edit
                      </button>
                    )}
