...
import { ReleaseToDraftPopup } from '../../components/procurement/ReleaseToDraftPopup';
...
const Procurement: React.FC = () => {
  ...
  const [releaseDraftReq, setReleaseDraftReq] = useState<ProcurementRequest | null>(null);
            ...
            <div className="space-y-4">
              ...
              {sideSection === 'Requests' && (
                <PrInboxView
                  ...
                  onStockAudit={(req) => setStockAuditReq(req)}
                  onDraftPO={(req) => setReleaseDraftReq(req)}
                />
              )}
              ...
              {/* §3B Stock Audit popup */}
              {stockAuditReq && (
                <StockAuditPopup
                  req={stockAuditReq}
                  onClose={() => setStockAuditReq(null)}
                  onSubmit={async (payload) => {
                    // Persists via the existing stock-check mechanism (surfaced in Inventory Audit).
                    // A dedicated AUD-XXXX audit entity + tracker arrives with View 4 (Phase 3).
                    const res = await updateProcurementRequestApi(stockAuditReq.id, {
                      stockCheckAssignedTo: payload.warehouseCodes.join(', '),
                      stockCheckStatus: 'Requested',
                      stockCheckDueDate: payload.targetDate,
                      stockCheckNotes: payload.comments || null,
                    });
                    if (res.success) {
                      await queryClient.refetchQueries({ queryKey: ['procurement-requests'] });
                      addToast('success', `Audit requested for ${stockAuditReq.code} @ ${payload.warehouseCodes.join(', ')} (target ${payload.targetDate}).`);
                      setStockAuditReq(null);
                    } else {
                      addToast('error', typeof res.error === 'string' ? res.error : 'Failed to submit audit request.');
                    }
                  }}
                />
              )}

              {/* §3C Release to Draft popup */}
              {releaseDraftReq && (
                <ReleaseToDraftPopup
                  req={releaseDraftReq}
                  onClose={() => setReleaseDraftReq(null)}
                  onPush={async (payload) => {
                    // Transitions the PR to PO Draft using the chosen PO date / computed expected.
                    // The full PO record (line items / GST / numbering) is created via the existing
                    // Draft-PO pipeline in Phase 2 (Purchase Orders).
                    const res = await updateProcurementRequestApi(releaseDraftReq.id, { status: 'PO Draft' });
                    if (res.success) {
                      await queryClient.refetchQueries({ queryKey: ['procurement-requests'] });
                      addToast('success', `${releaseDraftReq.code} released to Draft — PO ${payload.poDate} → expected ${payload.expectedDate} (+${payload.leadDays}d).`);
                      setReleaseDraftReq(null);
                    } else {
                      addToast('error', typeof res.error === 'string' ? res.error : 'Failed to release to draft.');
                    }
                  }}
                />
              )}

              {sideSection === 'Quotations' && <QuoteRequestsView />}
