  ...
  return (
    <Modal onClose={onClose} title={`Confirm Batch - ${formatUnifiedBatchLabel(batch)}`} subtitle={`${batch.productName} - ${batch.batchSize} KG`} size="lg">
      ...
      <div className="flex justify-end gap-2 mt-5 pt-4 border-t border-hairline">
        ...
        <button onClick={handleSave} className="inline-flex items-center gap-1.5 px-5 py-2 text-xs bg-brand hover:bg-brand text-white font-semibold rounded-lg shadow-sm transition-colors"><CheckCircle2 size={13} /> Confirm Batch</button>
      </div>
    </Modal>
  );
}
  ...
  return (
    <Modal onClose={onClose} title={`Adjust batch size — ${formatUnifiedBatchLabel(batch)}`} subtitle={`${batch.productName} · Current ${batch.batchSize} KG`} size="md">
      ...
      <div className="flex justify-end gap-2 mt-5 pt-4 border-t border-hairline">
        ...
        <button type="button" onClick={handleSave} className="inline-flex items-center gap-1.5 px-5 py-2 text-xs bg-brand hover:bg-brand text-white font-semibold rounded-lg shadow-sm transition-colors"><CheckCircle2 size={13} /> Save</button>
      </div>
    </Modal>
  );
}
