...
const CustomizationCatalog = () => {
  ...
  return (
    <div className="p-4 md:p-8 bg-surface-2/50 min-h-screen">
      <div className="mb-6">
        <h1 className="text-2xl md:text-3xl font-bold text-ink">Customization Catalog</h1>
        <div className="flex items-center gap-2 mt-2 text-sm bg-surface-3 px-4 py-2 rounded-lg">
          ...
          <span className="text-ink-2">Customization Catalog</span>
        </div>
      </div>

      <div className="bg-surface rounded-xl border border-hairline shadow-sm p-5 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <label className="text-sm font-medium text-ink-2">Care Type</label>
          <select
            value={care}
            onChange={(e) => setCare(e.target.value as CareType)}
            className="w-full sm:w-52 px-3 py-2 border border-border rounded-lg"
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
        <form onSubmit={handleSingleCreate} className="bg-surface rounded-xl border border-hairline shadow-sm p-5 space-y-3">
          <h2 className="text-lg font-semibold text-ink">Add Single Entry</h2>
          <input
            value={single.name}
          ...
          <button
            type="submit"
            disabled={submitting}
            className="px-4 py-2 bg-ink text-white rounded-lg disabled:opacity-60"
          >
            {submitting ? 'Saving...' : 'Create Entry'}
          </button>
        </form>

        <div className="bg-surface rounded-xl border border-hairline shadow-sm p-5 space-y-3">
          <h2 className="text-lg font-semibold text-ink">Bulk Add (Paste Blocks)</h2>
          ...
          <button
            ...
            className="px-4 py-2 bg-warn text-white rounded-lg disabled:opacity-60"
          >
      ...
      {message && (
        <div className="mb-6 rounded-lg border border-border bg-surface-2 px-4 py-3 text-sm text-ink-2">
          {message}
        </div>
      )}
      ...
      <div className="bg-surface rounded-xl border border-hairline shadow-sm p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-ink">Existing Entries</h2>
          <button type="button" onClick={() => void load()} className="text-sm text-ink-2 hover:underline">
            Refresh
          </button>
        </div>
        {loading ? (
          ...
          <>
            <div className="text-xs text-ink-3 mb-3">Total: {rows.length}</div>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 mb-5">
              {Object.entries(grouped).map(([k, count]) => (
                <div key={k} className="border border-border rounded-lg px-3 py-2 text-sm">
                  <span className="font-medium">{k}</span> <span className="text-ink-3">({count})</span>
                </div>
              ))}
            </div>
            <div className="overflow-auto max-h-[70vh]">
              <table className="w-full text-sm">
                <thead className="sticky top-0 z-20">
                  <tr className="[&_th]:bg-surface-2 text-left border-b border-border">
                ...
                <tbody>
                  {rows.slice(0, 100).map((row) => (
                    <tr key={row.custom_id} className="border-b border-hairline">
