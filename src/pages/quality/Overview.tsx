    ...
    <div className="flex-1 overflow-y-auto bg-slate-50 p-6 sm:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 mt-1">Quality Overview</h1>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="text-3xl font-bold text-slate-900 mt-2">{loading ? '—' : counts.inboundQc}</p>
            <p className="text-xs text-slate-500 mt-1">Sent from warehouse · pending QC</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-rose-600">Quarantine</p>
            <p className="text-3xl font-bold text-slate-900 mt-2">{loading ? '—' : counts.quarantine}</p>
            <p className="text-xs text-slate-500 mt-1">Awaiting Send to QC from warehouse</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-emerald-600">QC Passed</p>
            <p className="text-3xl font-bold text-slate-900 mt-2">{loading ? '—' : counts.history}</p>
            <p className="text-xs text-slate-500 mt-1">Completed QC inspections</p>
          </div>
        </div>
      </div>
    </div>
