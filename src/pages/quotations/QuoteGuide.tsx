...
export default function QuoteGuide() {
  ...
  return (
    <div className="pt-4 md:pt-6 space-y-6">
      ...
      {/* in-page nav */}
      <div className="bg-surface rounded-lg shadow-sm border border-hairline p-3 flex flex-wrap gap-2">
        {SECTIONS.map(([id, label]) => (
          <a key={id} href={`#${id}`} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-surface-3 text-ink-2 hover:bg-ink hover:text-white transition-all">{label}</a>
        ))}
      </div>
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3">
          {GLOSSARY.map(([term, def]) => (
            <div key={term} className="flex gap-3">
              <dt className="font-semibold text-ink whitespace-nowrap min-w-[7rem]">{term}</dt>
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm">
          {['RM', 'PM', 'Conversion', 'Overhead', 'Credit'].map((x, i) => (
            <span key={x} className="inline-flex items-center gap-2">
              <span className="px-3 py-2 bg-surface-2 border border-border rounded-lg font-medium text-ink-2">{x}</span>
              {i < 4 && <span className="text-ink-4">+</span>}
            </span>
          ))}
          <span className="text-ink-4 mx-1">→</span>
          <span className="px-3 py-2 bg-surface-3 border border-border rounded-lg font-semibold text-ink">Total Cost</span>
          <span className="text-ink-4 mx-1">× (1 + Markup) →</span>
          <span className="px-3 py-2 bg-ok-soft border border-ok rounded-lg font-bold text-ok">Sell Price</span>
        </div>
      </Section>
      ...
      <Section id="formulas" title="Formulas">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {FORMULAS.map((f) => (
            <div key={f.title} className="border border-border rounded-lg overflow-hidden">
              <div className="px-4 py-2 bg-surface-2 border-b border-hairline text-sm font-semibold text-ink">{f.title}</div>
              <pre className="p-4 text-xs text-ink-2 font-mono whitespace-pre-wrap leading-relaxed">{f.lines.join('\n')}</pre>
            </div>
          ))}
        </div>
      </Section>
      ...
      {loading ? (
        <div className="p-8 text-center"><Loader2 className="w-6 h-6 mx-auto text-ink-4 animate-spin" /><p className="text-sm text-ink-4 mt-2">Loading live configuration…</p></div>
      ) : (
        <>
          {/* Grades (live) */}
              ...
              {grades.map((g) => (
                <div key={g.id} className="border border-border rounded-lg p-4">
                  ...
                  <table className="w-full text-xs">
                    <tbody>
                      <tr><td className="text-ink-4 py-0.5">MOQ</td><td className="text-ink-2">{g.moq_labels[0]} → {g.moq_labels[6]}</td></tr>
                      <tr><td className="text-ink-4 py-0.5">Markup</td><td className="text-ink-2">{pct0(g.markups[0])} → {pct0(g.markups[6])}</td></tr>
                      <tr><td className="text-ink-4 py-0.5">QC days</td><td className="text-ink-2">{g.qc_days ?? '—'}</td></tr>
                    </tbody>
                  </table>
                </div>
              ))}
          ...
          <Section id="conversion" title="Conversion Cost Tables (₹/unit)">
            <p className="text-xs text-ink-3 mb-3">Fixed operational rates by packaging type, MOQ band, and volume bracket. A band's grade <code>bmap</code> factor scales these; no-monocarton subtracts ₹0.70.</p>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {CONVERSION_TABLES.map((t) => (
                <div key={t.name} className="border border-border rounded-lg overflow-hidden">
                  <div className="px-3 py-2 bg-surface-2 text-xs font-semibold text-ink">{t.name}</div>
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 z-20"><tr className="text-ink-4 [&_th]:bg-surface-2">{t.cols.map((c, i) => <th scope="col" key={c} className={`py-1.5 px-2 ${i > 0 ? 'text-right' : 'text-left'}`}>{c}</th>)}</tr></thead>
                    <tbody className="divide-y divide-hairline">
                      {t.rows.map((r) => <tr key={r[0] as string}>{r.map((c, i) => <td key={i} className={`py-1 px-2 ${i > 0 ? 'text-right text-ink-2' : 'text-ink-2'}`}>{c}</td>)}</tr>)}
                    </tbody>
                  </table>
                </div>
              ))}
          ...
          <Section id="overheads" title="Overhead Heads — baseline (live)">
            <div className="border border-border rounded-lg overflow-auto max-h-[70vh]">
              <table className="w-full text-xs">
                <thead className="sticky top-0 z-20"><tr className="text-ink-4 border-b border-hairline bg-surface-2 [&_th]:bg-surface-2">
                ...
                <tbody className="divide-y divide-hairline">
                  {overheads.map((r) => <tr key={r.id}><td className="py-1 px-3 text-ink-2">{r.head_name}</td>{r.band_values.map((v, i) => <td key={i} className="py-1 px-2 text-right text-ink-3">{Number(v).toFixed(2)}</td>)}</tr>)}
                  <tr className="bg-surface-2 font-semibold"><td className="py-1.5 px-3 text-ink-2">Total</td>{ohTotals.map((t, i) => <td key={i} className="py-1.5 px-2 text-right text-ink">{t.toFixed(2)}</td>)}</tr>
                </tbody>
              </table>
            </div>
          </Section>
          ...
          <Section id="timeline" title="Timeline Rules (live)">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="border border-border rounded-lg overflow-auto max-h-[70vh]">
                <div className="px-3 py-2 bg-surface-2 text-xs font-semibold text-ink">Manufacturing — days by band</div>
                <table className="w-full text-xs">
                  <thead className="sticky top-0 z-20"><tr className="text-ink-4 [&_th]:bg-surface-2"><th scope="col" className="py-1.5 px-2 text-left">Type</th>{[0, 1, 2, 3, 4, 5, 6].map((b) => <th scope="col" key={b} className="py-1.5 px-2 text-right">B{b}</th>)}</tr></thead>
                  <tbody className="divide-y divide-hairline">
                    {mfgTypes.map((t) => <tr key={t}><td className="py-1 px-2 text-ink-2">{t}</td>{mfgGrid(t).map((d, i) => <td key={i} className="py-1 px-2 text-right text-ink-3">{d}</td>)}</tr>)}
                  </tbody>
                </table>
              </div>
        ...
        <dl className="space-y-2">
          {WARNINGS.map(([w, d]) => (
            <div key={w} className="flex gap-3 text-sm">
              <dt className="font-medium text-warn min-w-[12rem]">{w}</dt><dd className="text-ink-2">{d}</dd>
            </div>
          ))}
        </dl>
      </Section>
...
function Section({ id, title, children }: { id: string; title: string; children: React.ReactNode }) {
  return (
    <section id={id} className="bg-surface rounded-lg shadow-sm border border-hairline p-5 scroll-mt-20">
...
function MiniTable({ title, cols, rows }: { title: string; cols: string[]; rows: string[][] }) {
  return (
    <div className="border border-border rounded-lg overflow-auto max-h-[70vh]">
      <div className="px-3 py-2 bg-surface-2 text-xs font-semibold text-ink">{title}</div>
      <table className="w-full text-xs">
        <thead className="sticky top-0 z-20"><tr className="text-ink-4 [&_th]:bg-surface-2">{cols.map((c, i) => <th scope="col" key={c} className={`py-1.5 px-2 ${i > 0 ? 'text-right' : 'text-left'}`}>{c}</th>)}</tr></thead>
        <tbody className="divide-y divide-hairline">
          {rows.map((r, ri) => <tr key={ri}>{r.map((c, i) => <td key={i} className={`py-1 px-2 ${i > 0 ? 'text-right text-ink-3' : 'text-ink-2'}`}>{c}</td>)}</tr>)}
        </tbody>
      </table>
    </div>
  );
