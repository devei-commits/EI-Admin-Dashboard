    ...
    <div className="space-y-4">
        ...
        <div className="border border-gray-200 rounded-lg overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100 bg-gray-50">
              ...
              {BAND_LABELS.map((b) => <th key={b} className="py-2.5 px-2 text-right">{b}</th>)}
              ...
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  {row.band_values.map((v, i) => (
                    <td key={i} className="py-1.5 px-1"><input className={`${inputClassName} py-1 px-2 w-16 text-right`} type="number" step="0.01" value={v} onChange={(e) => editBand(row.id, i, e.target.value)} /></td>
                  ))}
                  <td className="py-1.5 px-3 text-right whitespace-nowrap">
                    {dirty.has(row.id) && <button onClick={() => saveRow(row)} className="text-emerald-600 hover:text-emerald-700 p-1" title="Save"><Save className="w-4 h-4" /></button>}
                    <button onClick={() => setToDelete(row)} className="text-gray-400 hover:text-red-500 p-1" title="Delete"><Trash2 className="w-4 h-4" /></button>
                  </td>
                </tr>
              ))}
      ...
      {adding ? (
        <div className="border border-gray-200 rounded-lg p-3 flex items-end gap-2 flex-wrap">
          <input className={`${inputClassName} w-48`} placeholder="Head name" value={newName} onChange={(e) => setNewName(e.target.value)} />
          {newVals.map((v, i) => <input key={i} className={`${inputClassName} w-16 text-right`} type="number" step="0.01" value={v} onChange={(e) => setNewVals((p) => p.map((x, j) => j === i ? e.target.value : x))} title={BAND_LABELS[i]} />)}
